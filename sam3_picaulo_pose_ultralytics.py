"""Segment pallets from an RGB-D capture and export 3D poses using Ultralytics' SAM3.

Uses Ultralytics' internal text-promptable SAM3SemanticPredictor (not the public
`ultralytics.SAM` class, which only supports box/point prompts) so the same
"pallet" text prompt used with Meta's sam3 package can be reused here.
"""
...
def run_sam3_semantic_segmentation(
    checkpoint_path: Path, image_path: Path, prompt: str, confidence_threshold: float, image_size: int
):
    """Run Ultralytics' text-promptable SAM3 predictor and return (masks, plotted_result_image)."""
    predictor = SAM3SemanticPredictor(
        overrides=dict(model=str(checkpoint_path), task="segment", mode="predict", imgsz=image_size, conf=confidence_threshold)
    )
    results = predictor(source=str(image_path), text=[prompt])
    result = results[0]
    if result.masks is None:
        return [], result
    masks = result.masks.data.detach().cpu().numpy() > 0.5
    return list(masks), result
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", type=Path, default=PROJECT_ROOT / "sam3.pt")
...
def main() -> None:
    ...
    for path, name in ((args.input_dir, "Input folder"), (image_path, "RGB image"), (depth_map_path, "Depth map"), (args.checkpoint, "SAM3 checkpoint")):
        if not path.exists():
    ...
    print(f"Loading local SAM3 checkpoint via Ultralytics: {args.checkpoint}")
    binary_masks, detection_result = run_sam3_semantic_segmentation(
        args.checkpoint, image_path, args.prompt, args.confidence_threshold, args.imgsz
    )
    print(f"found {len(binary_masks)} object(s)")

    detection_path = output_dir / "sam3_detections.png"
    cv2.imwrite(str(detection_path), detection_result.plot())
    ...
    display(overlay, f"SAM3 (Ultralytics) masks: '{args.prompt}' ({len(binary_masks)} object(s))", args.show)
